...
export class TTSTools {
  ...
  async ensureReady(): Promise<void> {
    await this.ttsService.ensureReady();
  }
  ...
  async synthesizeText(params: {
    ...
    if (rawReferenceAudioPath) {
      // Reference audio validation is handled by the engine itself
      resolvedReferenceAudioPath = rawReferenceAudioPath;
    }
  ...
  inputSchema: {
    ...
    properties: {
      ...
      speed: {
        type: "number",
        description:
          "Speech speed (0.5 to 2.0, default: 1.0, Kokoro only).",
        minimum: 0.5,
        maximum: 2.0,
      },
      language: {
        type: "string",
        description:
          "Language code (e.g., 'en-us', 'en-gb', 'fr-fr', 'ja', 'cmn', default: 'en-us', Kokoro only).",
      },
      voice: {
        type: "string",
        description:
          "Voice name (e.g., 'af_sarah', 'am_adam', 'bf_emma', 'jf_alpha', default: 'af_sarah', Kokoro only). Can also blend voices using 'voice1:weight,voice2:weight' format.",
      },
      model_path: {
        type: "string",
        description:
          "Path to Kokoro ONNX model file (default: 'kokoro-v1.0.onnx', Kokoro only).",
      },
      voices_path: {
        type: "string",
        description:
          "Path to Kokoro voices bin file (default: 'voices-v1.0.bin', Kokoro only).",
      },
    },
    required: ["text"],
  },
